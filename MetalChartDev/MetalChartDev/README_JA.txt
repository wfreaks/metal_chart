...
ただしライブラリの目標上、少数のデータを装飾して可視化する機能は少なく、そういった機能が欲しい場合は
別のライブラリを使う（か独自に機能追加する）事が手軽と思われる.
UI操作に対応する必要のないパイチャートなどは、別の選択肢を考慮する事を勧める.
...
時間的な余裕が無い場合は、(3)か(4)の選択を迫られるし、
よほど腕の良いプログラマが余裕をもって担当したのでなければ、多くは(3)の選択が悲惨な結果へ導く確率は高いと考え、(4)を選択する。

私はShinobiChartを使う機会が多かった。そしてそれこそがライブラリ作成の契機である。
...
　整理されていなくて、ひどく泥臭く感じる.
・クラスの理想的な振る舞いがIFから明確でないため、実装が変わった時に挙動が変化する事がある
