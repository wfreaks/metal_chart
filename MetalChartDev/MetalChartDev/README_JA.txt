...
ライブラリ内でプロトコル化されていない、必ず使用する必要のあるクラスとプロトコルはほぼ全て「コアクラス」として FMChart.h に宣言されている。
（ほぼ、と書いたのは、これらの中に内部的にシェーダで使用する構造体のバッファを保持するものが含まれるためである.
  もちろんシェーダから全部自分で書けばその限りではない. また, iOS8での実装を可能とするために、インタフェースレベルでMTKViewと互換なViewをバイパス
　バイパスするためのクラスおよびタイプエイリアスが含まれるが、これらも例外である.）

概念はシンプルで、
(1) データ空間からビュー空間への写像を定義するプロトコル
(2) 紐付けた写像を受け取って描画するメソッドを定義するプロトコル
(3) 画面に追加で描画できるアタッチメントのためのプロトコル
(4) ComputeShaderを含む、フレーム描画ルーチン内で実行したいコードを挿入するためのプロトコル
(5) それらを管理するコンテナとしてのFMMetalChartクラス
で構成されている。
...
またより低レベルなコンポーネントの使い方も、FMChartConfiguratorのソース自体が、シナリオ毎の解説となるはずだ.


まずは/MetalChartDev/ViewController.swiftを見て欲しい.
